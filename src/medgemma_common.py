...
PROMPT_VERSION = "bc5cdr-medgemma-v1"
...
Follow the requested BC5CDR-style task exactly. Do not infer entities that are
not explicitly written in the source text. Return only data that follows the
...
2. DISEASE means an explicit disease, disorder, pathological condition, sign,
   or symptom treated as a disease mention in this biomedical context.
3. CHEMICAL means an explicit drug, chemical, compound, element, or named
   chemical class.
...
def tie_break_prompt(
    *,
    ...
    return f"""Decide whether one proposed BC5CDR entity should be accepted.
